...
jerk_goal(1,1) = 0.7;          % control target for jerk 1
jerk_goal(2,1) = 0.3;          % control target for jerk 2
...
kp(1,1) = 0.5;                 % proportional gain
kp(2,1) = 0.5;                   % proportional gain
ki(1,1) = 0.1;                 % integral gain
ki(2,1) = 0;                   % integral gain

k = 1;                         % discrete time counter
max_k = 5000;                  % max number of iterations
err = zeros(3,k);              % errors of [Neighbor; Global]
...
while (~converge) 
    ...
    avg(k) = mean(x(1:N,k));
    err(1,k) = jerk_goal(1,1) - avg(k); %Error 1
    err(2,k) = jerk_goal(2,1) - avg(k); %Error 2

    if (k>1)
        err(3,k) = err(3,k-1)+err(1,k);
    end
...
xlim([0 k+k/10])
limyu = max(x(jerk_node(1,1),:))+0.15;
